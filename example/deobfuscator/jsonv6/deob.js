...
class Deob {
  ...
  record(i) {
    if (this.isWriteFile) {
      fs.writeFile(__dirname + `/code_${i}.js`, this.code)
    }
  }
  ...
  prettierCode() {
    ...
    fs.writeFile(__dirname + '/pretty.js', newCode)
  }
        ...
        p.traverse({
          'FunctionDeclaration|VariableDeclarator'(path) {
            ...
            if (binding.referencePaths.length > count) {
              ...
              let parent = binding.path.find((p) => p.isFunctionDeclaration() || p.isVariableDeclaration())
              if (!parent) return
      ...
      CallExpression(path) {
        if (t.isFunctionExpression(path.node.callee)) {
          ...
          innerArguments.forEach((argument, index) => {
            path.get('callee').get('body').get('body')[0].get('argument').get('callee')
              .traverse({
                Identifier(p) {
                  if (p.parentKey !== 'params' && p.node.name === argument.name) {
    ...
    traverse(this.ast, {
      SwitchStatement(path) {
        ...
        fnBlockStatementPath.traverse({
          MemberExpression(path) {
            if (t.isStringLiteral(path.node.property) && path.node.property.value === 'split') {
    ...
    traverse(this.ast, {
      ...
      CallExpression(path) {
        if (path.node.callee.type === 'MemberExpression') {
          if (!['setTimeout', 'setInterval'].includes(path.node.callee.property?.value))
            return
          path.addComment('leading', label, true)
          return
        }
...
; (async function () {
  let rawCode = await fs.readFile(__dirname + '/code.js', { encoding: 'utf-8' })
  ...
  let deob = new Deob({
    ...
    isWriteFile: true, // 写出文件用作对比
  })

  deob.prettierCode(rawCode)
