...
export class AstDec {
      ...
      Program(p) {
        p.traverse({
          'FunctionDeclaration|VariableDeclarator'(path) {
            ...
            if (binding.referencePaths.length > decFunctionCallCount) {
   ...
   * var obj = { a: 123 } => var obj = { "a": 123 }  用于后续遍历使用
   */
  ...
  changeObjectAccessMode() {
    traverse(this.ast, {
      ...
      ObjectProperty(path) {
        if (t.isStringLiteral(path.key)) {
          let name = path.key.name
          path.key = t.identifier(name)
        }
        path.node.computed = false
      },
    })
