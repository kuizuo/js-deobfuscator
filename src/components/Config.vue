    ...
    <div sm="w-full 100">
      <div border="1 solid" class="rounded-sm">
        <div p="x-2 y-1" border="b gray-400/10">
          <span>配置列表</span>
          <Icon icon="ant-design:delete-outlined" size="24" class="float-right" @click="clearConfig"></Icon>
          <Icon icon="ant-design:reload-outlined" size="24" class="float-right" @click="reset"></Icon>
        </div>
        <div p="x-1 y-1">
          <draggable
            v-model="configList"
            ...
            <template #item="{ element }">
              <li :class="`list-group-item ${element.disable ? 'disable' : ''}`" :key="element.id">
                ...
                <span class="flex float-right">
                  <Icon icon="fe:disabled" color="#9e9e9e" size="24" @click="disableConfig(element)"></Icon>
                  <Icon icon="clarity:remove-line" color="#9e9e9e" size="24" @click="removeConfig(element)"></Icon>
                </span>
              </li>
            </template>
          </draggable>
      ...
      <div class="flex flex-wrap gap-2">
        <div v-for="item in configData">
          <el-tooltip class="box-item" :content="item.content" placement="top">
            <el-button class="w-36" @click="addConfig(item)">{{ item.name }}</el-button>
          </el-tooltip>
        </div>
      </div>
    </div>
