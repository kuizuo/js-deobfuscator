...
class AstEnc {
  constructor(setting) {
    let { jscode, encryptFunc, decryptFunc } = setting
    if (!jscode) throw new Error('请载入js代码')
  ...
  astConcatArrayUnshift(ast) {
    ...
    let shuffleAstUtil = new AstEnc({ jscode: shuffleCode })
...
function main() {
  ...
  // 改变对象属性访问方式
  astEnc.changePropertyAccess()
  astEnc.changeClassPropertyAccess()
  astEnc.changeBuiltinObjects()
  astEnc.stringToHex()

  astEnc.binaryToFlowerCode()
  astEnc.TemplateEncrypt()
  astEnc.stringEncrypt()

  // //读取还原数组乱序的代码
  astEnc.arrayShuffle()
  astEnc.astConcatArrayUnshift()
  astEnc.unshiftArrayDeclaration()

  astEnc.renameIdentifier()
  astEnc.appointedCodeLineEncrypt()
  astEnc.appointedCodeLineAscii()
  astEnc.numericEncrypt()

  let code = astEnc.code
